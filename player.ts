...
export function createPlayerMeshes(scene: THREE.Scene, playerUnits: number, PLAYER_Y: number): void {
  // Remove old meshes
...
export function updatePlayerFormation(targetX: number, PLAYER_MOVE_SPEED: number, delta: number) {
  if (playerMeshes.length > 0) {
    let sumX = 0;
...
export function shootFromAll(scene: THREE.Scene, playerMeshes: THREE.Mesh[], PLAYER_Y: number, shootBulletFrom: (x: number) => void) {
