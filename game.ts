...
let renderer: THREE.WebGLRenderer | undefined, scene: THREE.Scene, camera: THREE.PerspectiveCamera, animationId: number;
...
let enemyGroups: EnemyGroup[] = [], enemyLabels: THREE.Sprite[] = [], powerups: Powerup[] = [], powerupLabels: THREE.Sprite[] = [];
...
function createTextSprite(message: string, color = '#fff', fontSize = 48): THREE.Sprite {
...
export function initGame(container: HTMLElement): void {
  ...
  const leftLaneMaterial = new THREE.MeshBasicMaterial({ color: ENEMY_LANE_COLOR, transparent: true, opacity: 0.1 });
  const rightLaneMaterial = new THREE.MeshBasicMaterial({ color: POWERUP_LANE_COLOR, transparent: true, opacity: 0.1 });
  ...
  for (let i = 0; i < ENEMY_COUNT; i++) {
    ...
    enemyGroups.push({ mesh: enemyMeshes[0], size: enemySize, y, meshes: enemyMeshes });
  ...
  for (let i = 0; i < POWERUP_COUNT; i++) {
    ...
    scene.add(powerup);
    powerups.push({ mesh: powerup, hitCount, y, type: isFireRate ? 'fireRate' : 'units' });
    // Add label
    const label = createTextSprite(hitCount.toString(), isFireRate ? '#fff' : '#222', 48);
  ...
  function animate(): void {
    ...
    for (let i = bullets.length - 1; i >= 0; i--) {
      ...
      for (let j = enemyGroups.length - 1; j >= 0; j--) {
        ...
        if (Math.abs(b.mesh.position.x - group.mesh.position.x) < 0.8 && Math.abs(b.mesh.position.y - group.mesh.position.y) < 1) {
          group.size--;
      ...
      for (let j = powerups.length - 1; j >= 0; j--) {
        const p = powerups[j];
        if (Math.abs(b.mesh.position.x - p.mesh.position.x) < 0.8 && Math.abs(b.mesh.position.y - p.mesh.position.y) < 1) {
          p.hitCount--;
          // Update the powerup label to show the new hitCount
          if (powerupLabels[j] && powerupLabels[j].material.map) {
            const newLabel = createTextSprite(p.hitCount.toString(), p.type === 'fireRate' ? '#fff' : '#222', 48);
            powerupLabels[j].material.map = new THREE.CanvasTexture(newLabel.material.map!.image);
          }
          scene.remove(b.mesh);
          bullets.splice(i, 1);
          if (p.hitCount <= 0) {
